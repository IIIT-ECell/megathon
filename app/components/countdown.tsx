...
export default function Countdown({ target }: { target: string }) {
  ...
  return (
    <div
      role="timer"
      aria-live="off"
      className="grid w-full max-w-xl grid-cols-2 gap-px overflow-hidden rounded-2xl border border-black/[.08] bg-black/[.08] backdrop-blur-sm sm:grid-cols-4 dark:border-white/[.14] dark:bg-white/[.14]"
    >
      {units.map((unit) => (
        <div
          key={unit.label}
          className="flex flex-col items-center gap-1 bg-white/70 px-4 py-5 dark:bg-black/70"
        >
          ...
          <span className="text-[0.7rem] font-medium uppercase tracking-[0.16em] text-zinc-500 dark:text-zinc-400">
            {unit.label}
          </span>
        </div>
      ))}
      ...
      {live && (
        <p className="col-span-2 bg-white/70 px-4 pb-5 text-center text-sm font-medium text-orange-600 sm:col-span-4 dark:bg-black/70 dark:text-orange-400">
