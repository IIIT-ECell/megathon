            ...
            if (win.scrollTop() < change) {
                hero_overlay.css({
                    opacity: function() {
                        let opacity =
                            0.5 +
                            0.5 * (1 - (change - win.scrollTop()) / change);
...
let bannerClose = $("#closeBanner");
let shouldDisplayBannerKey = "display-megathon-banner";
bannerClose.click(function(event) {
    localStorage.setItem(shouldDisplayBannerKey, "false");
    banner.fadeOut();
});
if (localStorage.getItem(shouldDisplayBannerKey) === "false") {
    banner.fadeOut();
}
